...
def test_protect_store(
    client: TestClient,
    db: Session,
    admin_headers: dict[str, str],
    test_data: s.TestData,
):
    db_stores = db.scalars(sa.select(m.Store)).all()
    assert db_stores
    ...
    assert response.status_code == 200
    assert response.json()["is_protected"] is False

    user_subscriptions = db.scalars(sa.select(m.Subscription).where(m.Subscription.user_id == store.user.id)).all()
    assert user_subscriptions

    # add test subscription_stripe_id
    subscription_stripe_id = "sub_1MowQVLkdIwHu7ixeRlqHVzs"
    user_subscriptions[-1].subscription_stripe_id = subscription_stripe_id
    db.commit()
    db.refresh(store)
    assert user_subscriptions[-1].subscription_stripe_id == subscription_stripe_id
