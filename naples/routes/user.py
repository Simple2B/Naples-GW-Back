...
# admin contact data for template
@user_router.get(
    "/contacts",
    status_code=status.HTTP_200_OK,
...
def get_admin_contacts_data(
    db: Session = Depends(get_db),
):
    """Get admin information for tamplate"""

    admin = db.scalar(sa.select(m.User).where(m.User.role == s.UserRole.ADMIN.value))

    if not admin:
        log(log.ERROR, "Admin not found")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Admin not found")

    return s.AdminContactData(
        email=admin.email,
        phone=admin.phone,
...
def update_admin_contacts_data(
    data: s.AdminContactDataIn,
    db: Session = Depends(get_db),
    admin: m.User = Depends(get_admin),
):
    """Update admin information for tamplate"""

    if data.email:
        log(log.INFO, f"Admin {admin.email} updated his email to {data.email}")
        admin.email = data.email

    if data.phone:
        log(log.INFO, f"Admin {admin.email} updated his phone to {data.phone}")
    ...
    log(log.INFO, f"Admin {admin.email} updated his contact data")
